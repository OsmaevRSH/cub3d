...
int					ft_get_resolution_size(t_mlx *mlx, const char *str_3, int *flag, t_screen_size *data);
void				ft_get_map(t_check flag, char *line, int fd, t_mlx *mlx);
void				check_count_player_in_map(const char *str, t_mlx *mlx);
void				ft_get_all_param_without_map(t_mlx *mlx, char **split_str, t_check *flag, char *line);
